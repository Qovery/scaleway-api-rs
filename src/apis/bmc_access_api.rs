...
pub async fn get_bmc_access(
    ...
    zone: &str,
    server_id: &str,
) -> Result<models::ScalewayPeriodBaremetalPeriodV1PeriodBmcAccess, Error<GetBmcAccessError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodBmcAccess`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodBmcAccess`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetBmcAccessError> = serde_json::from_str(&content).ok();
...
pub async fn start_bmc_access(
    ...
    zone: &str,
    server_id: &str,
    start_bmc_access_request: models::StartBmcAccessRequest,
) -> Result<models::ScalewayPeriodBaremetalPeriodV1PeriodBmcAccess, Error<StartBmcAccessError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_zone = zone;
    let p_server_id = server_id;
    let p_start_bmc_access_request = start_bmc_access_request;
    ...
    req_builder = req_builder.json(&p_start_bmc_access_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<StartBmcAccessError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn stop_bmc_access(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<StopBmcAccessError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
