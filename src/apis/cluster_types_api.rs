...
/// struct for typed errors of method [`list_cluster_types`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListClusterTypesError {
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodK8sPeriodV1PeriodListClusterTypesResponse`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodK8sPeriodV1PeriodListClusterTypesResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListClusterTypesError> = serde_json::from_str(&content).ok();
