...
/// struct for typed errors of method [`migrate_endpoint`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum MigrateEndpointError {
    UnknownValue(serde_json::Value),
}
...
pub async fn create_endpoint(
    ...
    region: &str,
    instance_id: &str,
    create_endpoint_request: models::CreateEndpointRequest,
) -> Result<models::ScalewayPeriodRdbPeriodV1PeriodEndpoint, Error<CreateEndpointError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_instance_id = instance_id;
    let p_create_endpoint_request = create_endpoint_request;
    ...
    req_builder = req_builder.json(&p_create_endpoint_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodRdbPeriodV1PeriodEndpoint`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodRdbPeriodV1PeriodEndpoint`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateEndpointError> = serde_json::from_str(&content).ok();
...
pub async fn delete_endpoint(
    ...
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_endpoint_id = endpoint_id;

    let uri_str = format!(
        "{}/rdb/v1/regions/{region}/endpoints/{endpoint_id}",
        configuration.base_path,
        region = crate::apis::urlencode(p_region),
        endpoint_id = crate::apis::urlencode(p_endpoint_id)
    );
    let mut req_builder = configuration
        .client
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DeleteEndpointError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn get_endpoint(
    ...
    region: &str,
    endpoint_id: &str,
) -> Result<models::ScalewayPeriodRdbPeriodV1PeriodEndpoint, Error<GetEndpointError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_endpoint_id = endpoint_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetEndpointError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn migrate_endpoint(
    ...
    region: &str,
    endpoint_id: &str,
    migrate_endpoint_request: models::MigrateEndpointRequest,
) -> Result<models::ScalewayPeriodRdbPeriodV1PeriodEndpoint, Error<MigrateEndpointError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_endpoint_id = endpoint_id;
    let p_migrate_endpoint_request = migrate_endpoint_request;
    ...
    req_builder = req_builder.json(&p_migrate_endpoint_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<MigrateEndpointError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
