...
pub async fn list_database_engines(configuration: &configuration::Configuration, region: &str, page: Option<f32>, page_size: Option<f32>) -> Result<crate::models::ScalewayRdbV1ListDatabaseEnginesResponse, Error<ListDatabaseEnginesError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/rdb/v1/regions/{region}/database-engines", configuration.base_path, region=crate::apis::urlencode(region));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ListDatabaseEnginesError> = serde_json::from_str(&local_var_content).ok();
