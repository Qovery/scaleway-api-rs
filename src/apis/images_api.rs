...
pub async fn delete_image(
    ...
    region: &str,
    image_id: &str,
) -> Result<models::ScalewayPeriodRegistryPeriodV1PeriodImage, Error<DeleteImageError>> {
    ...
    let uri_str = format!(
        ...
        image_id = crate::apis::urlencode(p_image_id)
    );
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodRegistryPeriodV1PeriodImage`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodRegistryPeriodV1PeriodImage`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_image(
    ...
    region: &str,
    image_id: &str,
) -> Result<models::ScalewayPeriodRegistryPeriodV1PeriodImage, Error<GetImageError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_image_id = image_id;
...
) -> Result<models::ScalewayPeriodRegistryPeriodV1PeriodListImagesResponse, Error<ListImagesError>>
{
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_page = page;
    ...
    if let Some(ref param_value) = p_namespace_id {
        req_builder = req_builder.query(&[("namespace_id", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodRegistryPeriodV1PeriodListImagesResponse`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodRegistryPeriodV1PeriodListImagesResponse`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn update_image(
    ...
    region: &str,
    image_id: &str,
    update_image_request: models::UpdateImageRequest,
) -> Result<models::ScalewayPeriodRegistryPeriodV1PeriodImage, Error<UpdateImageError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_image_id = image_id;
    let p_update_image_request = update_image_request;
    ...
    req_builder = req_builder.json(&p_update_image_request);
