...
pub async fn create_network(configuration: &configuration::Configuration, region: &str, inline_object88: crate::models::InlineObject88) -> Result<crate::models::ScalewayIotV1CreateNetworkResponse, Error<CreateNetworkError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/iot/v1/regions/{region}/networks", configuration.base_path, region=crate::apis::urlencode(region));
...
pub async fn delete_network(configuration: &configuration::Configuration, region: &str, network_id: &str) -> Result<(), Error<DeleteNetworkError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/iot/v1/regions/{region}/networks/{network_id}", configuration.base_path, region=crate::apis::urlencode(region), network_id=crate::apis::urlencode(network_id));
...
pub async fn get_network(configuration: &configuration::Configuration, region: &str, network_id: &str) -> Result<crate::models::ScalewayIotV1Network, Error<GetNetworkError>> {
...
pub async fn list_networks(configuration: &configuration::Configuration, region: &str, page: Option<f32>, page_size: Option<f32>, order_by: Option<&str>, name: Option<&str>, hub_id: Option<&str>, topic_prefix: Option<&str>) -> Result<crate::models::ScalewayIotV1ListNetworksResponse, Error<ListNetworksError>> {
    ...
    if let Some(ref local_var_str) = topic_prefix {
        local_var_req_builder = local_var_req_builder.query(&[("topic_prefix", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_user_agent) = configuration.user_agent {
        local_var_req_builder = local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
    }
