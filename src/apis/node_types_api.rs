...
pub async fn list_node_types(configuration: &configuration::Configuration, region: &str, include_disabled_types: Option<bool>, page: Option<f32>, page_size: Option<f32>) -> Result<crate::models::ScalewayRdbV1ListNodeTypesResponse, Error<ListNodeTypesError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/rdb/v1/regions/{region}/node-types", configuration.base_path, region=crate::apis::urlencode(region));
    let mut local_var_req_builder = local_var_client.get(local_var_uri_str.as_str());

    if let Some(ref local_var_str) = include_disabled_types {
        local_var_req_builder = local_var_req_builder.query(&[("include_disabled_types", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ListNodeTypesError> = serde_json::from_str(&local_var_content).ok();
