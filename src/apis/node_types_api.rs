...
/// struct for typed errors of method [`list_node_types`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListNodeTypesError {
...
) -> Result<models::ScalewayPeriodRdbPeriodV1PeriodListNodeTypesResponse, Error<ListNodeTypesError>>
{
    ...
    let p_region = region;
    let p_include_disabled_types = include_disabled_types;
    let p_page = page;
    ...
    req_builder = req_builder.query(&[(
        ...
        &p_include_disabled_types.to_string(),
    )]);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodRdbPeriodV1PeriodListNodeTypesResponse`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodRdbPeriodV1PeriodListNodeTypesResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListNodeTypesError> = serde_json::from_str(&content).ok();
