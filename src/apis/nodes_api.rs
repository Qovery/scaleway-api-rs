...
pub async fn create_external_node(
...
) -> Result<models::ScalewayPeriodK8sPeriodV1PeriodExternalNode, Error<CreateExternalNodeError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodK8sPeriodV1PeriodExternalNode`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodK8sPeriodV1PeriodExternalNode`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateExternalNodeError> = serde_json::from_str(&content).ok();
...
pub async fn delete_node(
    ...
    skip_drain: bool,
    replace: bool,
) -> Result<models::ScalewayPeriodK8sPeriodV1PeriodNode, Error<DeleteNodeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_node_id = node_id;
    let p_skip_drain = skip_drain;
    ...
    let uri_str = format!(
        ...
        configuration.base_path,
        region = crate::apis::urlencode(p_region),
        node_id = crate::apis::urlencode(p_node_id)
    );
    let mut req_builder = configuration
        .client
        .request(reqwest::Method::DELETE, &uri_str);

    req_builder = req_builder.query(&[("skip_drain", &p_skip_drain.to_string())]);
    req_builder = req_builder.query(&[("replace", &p_replace.to_string())]);
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
    if let Some(ref apikey) = configuration.api_key {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodK8sPeriodV1PeriodNode`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodK8sPeriodV1PeriodNode`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_node(
    ...
    region: &str,
    node_id: &str,
) -> Result<models::ScalewayPeriodK8sPeriodV1PeriodNode, Error<GetNodeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_node_id = node_id;
...
pub async fn list_nodes(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodK8sPeriodV1PeriodListNodesResponse`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodK8sPeriodV1PeriodListNodesResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListNodesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn reboot_node(
    ...
    node_id: &str,
    body: serde_json::Value,
) -> Result<models::ScalewayPeriodK8sPeriodV1PeriodNode, Error<RebootNodeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_node_id = node_id;
    let p_body = body;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<RebootNodeError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
