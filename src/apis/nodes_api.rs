...
pub async fn get_node(configuration: &configuration::Configuration, region: &str, node_id: &str) -> Result<crate::models::ScalewayK8sV1Node, Error<GetNodeError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/k8s/v1/regions/{region}/nodes/{node_id}", configuration.base_path, region=crate::apis::urlencode(region), node_id=crate::apis::urlencode(node_id));
...
pub async fn list_nodes(configuration: &configuration::Configuration, region: &str, cluster_id: &str, pool_id: Option<&str>, order_by: Option<&str>, page: Option<f32>, page_size: Option<f32>, name: Option<&str>, status: Option<&str>) -> Result<crate::models::ScalewayK8sV1ListNodesResponse, Error<ListNodesError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/k8s/v1/regions/{region}/clusters/{cluster_id}/nodes", configuration.base_path, region=crate::apis::urlencode(region), cluster_id=crate::apis::urlencode(cluster_id));
    let mut local_var_req_builder = local_var_client.get(local_var_uri_str.as_str());

    if let Some(ref local_var_str) = pool_id {
        local_var_req_builder = local_var_req_builder.query(&[("pool_id", &local_var_str.to_string())]);
    }
...
pub async fn reboot_node(configuration: &configuration::Configuration, region: &str, node_id: &str, body: serde_json::Value) -> Result<crate::models::ScalewayK8sV1Node, Error<RebootNodeError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/k8s/v1/regions/{region}/nodes/{node_id}/reboot", configuration.base_path, region=crate::apis::urlencode(region), node_id=crate::apis::urlencode(node_id));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<RebootNodeError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn replace_node(configuration: &configuration::Configuration, region: &str, node_id: &str, body: serde_json::Value) -> Result<crate::models::ScalewayK8sV1Node, Error<ReplaceNodeError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/k8s/v1/regions/{region}/nodes/{node_id}/replace", configuration.base_path, region=crate::apis::urlencode(region), node_id=crate::apis::urlencode(node_id));
    let mut local_var_req_builder = local_var_client.post(local_var_uri_str.as_str());
