...
pub async fn get_offer(configuration: &configuration::Configuration, zone: &str, offer_id: &str) -> Result<crate::models::ScalewayBaremetalV1Offer, Error<GetOfferError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/baremetal/v1/zones/{zone}/offers/{offer_id}", configuration.base_path, zone=crate::apis::urlencode(zone), offer_id=crate::apis::urlencode(offer_id));
...
pub async fn list_offers(configuration: &configuration::Configuration, zone: &str, page: Option<f32>, page_size: Option<f32>) -> Result<crate::models::ScalewayBaremetalV1ListOffersResponse, Error<ListOffersError>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/baremetal/v1/zones/{zone}/offers", configuration.base_path, zone=crate::apis::urlencode(zone));
    let mut local_var_req_builder = local_var_client.get(local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ListOffersError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
