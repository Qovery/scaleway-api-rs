...
pub async fn get_offer(
    ...
    zone: &str,
    offer_id: &str,
) -> Result<models::ScalewayPeriodBaremetalPeriodV1PeriodOffer, Error<GetOfferError>> {
    ...
    let uri_str = format!(
        ...
        offer_id = crate::apis::urlencode(p_offer_id)
    );
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodOffer`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodOffer`")))),
        }
    } else {
        let content = resp.text().await?;
...
) -> Result<models::ScalewayPeriodBaremetalPeriodV1PeriodListOffersResponse, Error<ListOffersError>>
{
    ...
    let p_subscription_period = subscription_period;
    let p_name = name;
    ...
    if let Some(ref param_value) = p_subscription_period {
        req_builder = req_builder.query(&[("subscription_period", &param_value.to_string())]);
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodListOffersResponse`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodListOffersResponse`")))),
        }
    } else {
        let content = resp.text().await?;
