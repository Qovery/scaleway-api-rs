...
pub async fn get_option(
    ...
    zone: &str,
    option_id: &str,
) -> Result<models::ScalewayPeriodBaremetalPeriodV1PeriodOption, Error<GetOptionError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodOption`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodOption`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetOptionError> = serde_json::from_str(&content).ok();
...
) -> Result<models::ScalewayPeriodBaremetalPeriodV1PeriodListOptionsResponse, Error<ListOptionsError>>
{
    ...
    if let Some(ref param_value) = p_offer_id {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodListOptionsResponse`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodListOptionsResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListOptionsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn list_settings(
    ...
    models::ScalewayPeriodBaremetalPeriodV1PeriodListSettingsResponse,
...
> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodListSettingsResponse`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodListSettingsResponse`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListSettingsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
...
pub async fn update_setting(
    ...
    zone: &str,
    setting_id: &str,
    update_setting_request: models::UpdateSettingRequest,
) -> Result<models::ScalewayPeriodBaremetalPeriodV1PeriodSetting, Error<UpdateSettingError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_zone = zone;
    let p_setting_id = setting_id;
    let p_update_setting_request = update_setting_request;

    let uri_str = format!(
        "{}/baremetal/v1/zones/{zone}/settings/{setting_id}",
        configuration.base_path,
        zone = crate::apis::urlencode(p_zone),
        setting_id = crate::apis::urlencode(p_setting_id)
    );
    ...
    req_builder = req_builder.json(&p_update_setting_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodSetting`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodSetting`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<UpdateSettingError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
