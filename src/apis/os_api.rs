...
/// struct for typed errors of method `get_os1`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetOs1Error {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method `list_os1`
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListOs1Error {
    UnknownValue(serde_json::Value),
}


/// Get an Operating System (OS).
pub async fn get_os1(configuration: &configuration::Configuration, zone: &str, os_id: &str) -> Result<crate::models::ScalewayAppleSiliconV1alpha1Os, Error<GetOs1Error>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/apple-silicon/v1alpha1/zones/{zone}/os/{os_id}", configuration.base_path, zone=crate::apis::urlencode(zone), os_id=crate::apis::urlencode(os_id));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetOs1Error> = serde_json::from_str(&local_var_content).ok();
...
pub async fn list_os1(configuration: &configuration::Configuration, zone: &str, page: Option<f32>, page_size: Option<f32>, server_type: Option<&str>, name: Option<&str>) -> Result<crate::models::ScalewayAppleSiliconV1alpha1ListOsResponse, Error<ListOs1Error>> {

    let local_var_client = &configuration.client;

    let local_var_uri_str = format!("{}/apple-silicon/v1alpha1/zones/{zone}/os", configuration.base_path, zone=crate::apis::urlencode(zone));
    let mut local_var_req_builder = local_var_client.get(local_var_uri_str.as_str());
    ...
    if let Some(ref local_var_str) = server_type {
        local_var_req_builder = local_var_req_builder.query(&[("server_type", &local_var_str.to_string())]);
    }
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<ListOs1Error> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
