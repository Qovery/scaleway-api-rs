...
/// struct for typed errors of method [`get_default_partitioning_schema`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetDefaultPartitioningSchemaError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`validate_partitioning_schema`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ValidatePartitioningSchemaError {
    UnknownValue(serde_json::Value),
}
...
pub async fn get_default_partitioning_schema(
    ...
    models::ScalewayPeriodBaremetalPeriodV1PeriodSchema,
    Error<GetDefaultPartitioningSchemaError>,
> {
    ...
    req_builder = req_builder.query(&[("offer_id", &p_offer_id.to_string())]);
    req_builder = req_builder.query(&[("os_id", &p_os_id.to_string())]);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodSchema`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodBaremetalPeriodV1PeriodSchema`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetDefaultPartitioningSchemaError> = serde_json::from_str(&content).ok();
...
pub async fn validate_partitioning_schema(
    configuration: &configuration::Configuration,
    zone: &str,
    validate_partitioning_schema_request: models::ValidatePartitioningSchemaRequest,
) -> Result<(), Error<ValidatePartitioningSchemaError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_zone = zone;
    let p_validate_partitioning_schema_request = validate_partitioning_schema_request;
    ...
    req_builder = req_builder.json(&p_validate_partitioning_schema_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ValidatePartitioningSchemaError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
