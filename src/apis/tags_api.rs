...
pub async fn delete_tag(
    ...
    tag_id: &str,
    force: Option<bool>,
) -> Result<models::ScalewayPeriodRegistryPeriodV1PeriodTag, Error<DeleteTagError>> {
    ...
    let uri_str = format!(
        ...
        tag_id = crate::apis::urlencode(p_tag_id)
    );
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodRegistryPeriodV1PeriodTag`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodRegistryPeriodV1PeriodTag`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_tag(
    ...
    region: &str,
    tag_id: &str,
) -> Result<models::ScalewayPeriodRegistryPeriodV1PeriodTag, Error<GetTagError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_region = region;
    let p_tag_id = tag_id;
...
pub async fn list_tags(
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::ScalewayPeriodRegistryPeriodV1PeriodListTagsResponse`"))),
            ContentType::Unsupported(unknown_type) => Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::ScalewayPeriodRegistryPeriodV1PeriodListTagsResponse`")))),
        }
    } else {
        let content = resp.text().await?;
