...
/// LbV1RegionsRegionFrontendsFrontendIdAclsMatch : The ACL match rule. You can have one of those three cases:    - `ip_subnet` is defined   - `http_filter` and `http_filter_value` are defined   - `ip_subnet`, `http_filter` and `http_filter_value` are defined 



#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct LbV1RegionsRegionFrontendsFrontendIdAclsMatch {
    /// A list of IPs or CIDR v4/v6 addresses of the client of the session to match
    #[serde(rename = "ip_subnet", skip_serializing_if = "Option::is_none")]
    pub ip_subnet: Option<Vec<String>>,
    /// The HTTP filter to match. This filter is supported only if your backend supports HTTP forwarding. It extracts the request's URL path, which starts at the first slash and ends before the question mark (without the host part). 
    #[serde(rename = "http_filter", skip_serializing_if = "Option::is_none")]
    pub http_filter: Option<HttpFilter>,
    /// A list of possible values to match for the given HTTP filter
    #[serde(rename = "http_filter_value", skip_serializing_if = "Option::is_none")]
    pub http_filter_value: Option<Vec<String>>,
...
impl LbV1RegionsRegionFrontendsFrontendIdAclsMatch {
    /// The ACL match rule. You can have one of those three cases:    - `ip_subnet` is defined   - `http_filter` and `http_filter_value` are defined   - `ip_subnet`, `http_filter` and `http_filter_value` are defined 
    pub fn new() -> LbV1RegionsRegionFrontendsFrontendIdAclsMatch {
        LbV1RegionsRegionFrontendsFrontendIdAclsMatch {
            ip_subnet: None,
            http_filter: None,
            http_filter_value: None,
            invert: None,
        }
    }
}
...
pub enum HttpFilter {
    #[serde(rename = "acl_http_filter_none")]
    AclHttpFilterNone,
    #[serde(rename = "path_begin")]
    PathBegin,
    #[serde(rename = "path_end")]
    PathEnd,
    #[serde(rename = "regex")]
    Regex,
}
